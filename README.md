...
## visionary
### What even is this
    - Dolby Vision is fucking complicated. I will do my best to explain the tradeoffs imposed by this script below but if you would also like your brain to be poisoned as mine is by the maniacal ravings of the lunatics running Dolby Laboratories, I invite you to visit [this page](https://professionalsupport.dolby.com/s/article/What-is-Dolby-Vision-Profile?language=en_US) and download the Dolby Vision Profiles and Levels PDF file. Beware: that which is learned cannot be un-learned.
    - It requires [dovi_tool](https://github.com/quietvoid/dovi_tool) and [hdr10plus_tool](https://github.com/quietvoid/hdr10plus_tool) in addition to ffmpeg
    - visionary _currently_ transcodes to HEVC as there is, to my knowledge, not even one instance anywhere in the world of AV1 media with dynamic metadata, Dolby Vision or HDR10+, despite Dolby Vision seeming to have a profile for it. While I'm not super keen on going HEVC to HEVC, crf encoding at objectively good rate factors still reduces the file size dramatically, so it still has quite strong value for storage reduction. ffmpeg is [working on](https://github.com/search?q=repo%3AFFmpeg%2FFFmpeg+Dolby+Vision&type=commits&s=committer-date&o=desc) native support for Dolby Vision, which appears to include an impementation of DV in AV1, so who knows.
    - Due to what I perceive as the nature of dual-layer (aka Full Enhancement Layer or FEL aka base-layer + enhancement-layer + rpu aka BL+EL+RPU) Dolby Vision files, the base layer and the enhancement layer have to be encoded in some kind of informed parallel process that aligns keyframes properly so the 1080p EL and the 2160p BL overlay correctly for decoding purposes. Since the EL is used as a kind of mask layer, it contains very strange video information indeed, and to date I have not figured out how to force x265 to replicate the frame pattern of a BL onto an EL. SO. All of that is to say, visionary can only output Minimum Enhancement Layer aka MEL aka BL+RPU files, in particular Dolby Vision Profile 8 Level 6, to be specific.
    - It is also worth noting that I have not figured out how to make ffmpeg deal with the proprietary colorspace of Dolby Vision Profile 5 (IPT-PQ-C) reliably, so I throw a warning when I see it, and the output files probably won't work. It is not uncommon for streaming content to be Profile 5, but not _all_ Dolby Vision streaming content is Profile 5 and not even all versions of the same piece of content from different streamers are in Profile 5 so if you have a Profile 5 stream, put it in the trash and go find a Profile 7 or 8 version, it's probably out there, and it's less stupid. Optionally, if you know how to make ffmpeg convert IPT-PQ-C to a sane colorspace as part of the transcoding chain, open an issue and educate me, I would very much like to support this input format but still output a profile 8 stream in a normal colorspace a sane person would choose.
    - This script can _optionally_ use the Dolby Vision Professional Verification Toolkit to "verify" the output file as a valid Dolby Vision Profile 8 Level 6 file (BL+RPU). It is _exceedingly rare_ to have zero errors or warnings, even on the original source files, so I would not necessarily recommend you get this toolkit and use this feature regularly. I baked it in while I was developing the script to make sure I was producing output that passed some basic checks, but you have to learn a great deal about what you can ignore to find the output meaningful and I would not recommend anyone look at enough output from it to learn that. It is truly useless esoterica.
    - This script leverages a fifo/named pipe to avoid having a third copy of the video bitstream lying about while it works. Since 4K bluray video streams are obscenely large, it means you only need ~2x the size of the original file for working set space instead of ~3x, so we've got that going for us. Which is nice.
### How do I use it
### Confusing options explained
